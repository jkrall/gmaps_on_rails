class GoogleMapMarker
  ...
  def to_js
    ...
    js << "#{map.dom_id}_markers['#{dom_id}'] = new GMarker(new GLatLng(#{lat}, #{lng})#{i});"
    if self.html
      js << "GEvent.addListener(#{map.dom_id}_markers['#{dom_id}'], 'click', function() { #{map.dom_id}_markers['#{dom_id}'].openInfoWindowHtml('#{escape_javascript(html)}') });"
    end

    js << "#{map.dom_id}.addOverlay(#{map.dom_id}_markers['#{dom_id}']);"
